import { GameObject } from "/js/snake/GameObject.js";
import { Wall } from "/js/snake/Wall.js";
import { Snake } from '/js/snake/Snake.js'
import { Score } from "/js/snake/Score.js";
export class GameMap extends GameObject {
    constructor(parent, kind) {
        super();
        this.parent = $('.' + parent)[0];
        this.$canvas = $('canvas')[0];
        this.ctx = this.$canvas.getContext('2d');
        this.rows = 16;
        this.cols = 16;
        this.L = 0;

        this.inner_walls_count = 20;
        this.walls = [];
        this.kind = kind;
        if (this.kind === 'double') {
            this.snakes = [
                new Snake({ id: 0, color: "#4876EC", r: this.rows - 2, c: 1 }, this),
                new Snake({ id: 1, color: "#F94848", r: 1, c: this.cols - 2 }, this),
            ];
        } else if (this.kind === 'single') {
            this.snakes = [
                new Snake({ id: 0, color: "#4876EC", r: this.rows - 2, c: 1 }, this),
            ];
            this.inner_walls_count = 0;

        }
        this.step = 0;

    }
    start() {
        for (let i = 0; i < 1000; i++)//暴力生成地图
        {
            if (this.create_walls()) break;
        }
        if (this.kind === 'single') {
            for (let i = 0; i < 1000; i++)//暴力生成score
            {
                if (this.create_score()) break;
            }
        }

        this.add_listening_events();
    }
    update_size() {

        this.L = parseInt(Math.min(this.parent.clientWidth / this.cols, this.parent.clientHeight / this.rows));
        this.ctx.canvas.width = this.L * this.cols;
        this.ctx.canvas.height = this.L * this.rows;

    }
    check_connectivity(g, sx, sy, tx, ty)//深度优先检查连通性
    {
        if (sx == tx && sy == ty) return true;
        g[sx][sy] = true;
        let dx = [-1, 0, 1, 0], dy = [0, 1, 0, -1];
        for (let i = 0; i < 4; i++) {
            let xx = sx + dx[i], yy = sy + dy[i];
            if (g[xx][yy]) continue;
            if (xx < 1 || xx > this.rows - 2 || yy < 1 || yy > this.cols - 2) continue;
            if (this.check_connectivity(g, xx, yy, tx, ty)) return true;
        }
        return false;
    }
    create_walls() {
        const g = [];//g为一个二维数组，标志着地图上某处是否有障碍物，true为有
        for (let r = 0; r < this.rows; r++) {
            g[r] = [];
            for (let c = 0; c < this.cols; c++) {
                g[r][c] = false;
            }
        }
        for (let r = 0; r < this.rows; r++) {
            g[r][0] = g[r][this.cols - 1] = true;
        }
        for (let c = 0; c < this.cols; c++) {
            g[0][c] = g[this.rows - 1][c] = true;
        }

        //创建内部的随机障碍物,必须是沿中心轴对称
        for (let i = 0; i < this.inner_walls_count/2; i++) {
            for (let j = 0; j < 1000; j++)//暴力生成障碍物
            {
                let r = parseInt(Math.random() * this.rows);
                let c = parseInt(Math.random() * this.cols);
                if (g[r][c] || g[this.rows - 1 - r][this.cols - 1 - c]) continue;
                if (r === this.rows - 2 && c === 1 || c == this.cols - 2 && r === 1) continue;
                g[r][c] = g[this.rows - 1 - r][this.cols - 1 - c] = true;

                break;

            }
        }
        const copy_g = JSON.parse(JSON.stringify(g));
        if (!this.check_connectivity(copy_g, this.rows - 2, 1, 1, this.cols - 2)) return false;

        for (let r = 0; r < this.rows; r++) {
            for (let c = 0; c < this.cols; c++) {
                if (g[r][c]) {
                    this.walls.push(new Wall(r, c, this));
                }
            }
        }
        return true;


    }
    create_score() {
        let r = parseInt(Math.random() * (this.rows - 1));
        let c = parseInt(Math.random() * (this.cols - 1));
        if (r === 0 || c === 0) return false;
        for (const snake of this.snakes) {
            let k = snake.cells.length;

            if (snake.status === "move" && !snake.check_tail_increasing())//当蛇尾会前进的时候，蛇尾就不用判断
            {
                k--;
            }
            for (let i = 0; i < k; i++) {
                if (snake.cells[i].r === r && snake.cells[i].c === c) return false;
            }
        }
        this.score = new Score({ r: r, c: c }, this);
        return true;
    }
    add_listening_events() {
        this.ctx.canvas.focus();
        if (this.kind === 'double') {
            const [snake0, snake1] = this.snakes;
            this.ctx.canvas.addEventListener("keydown", e => {

                if (e.key === 'w') snake0.set_direction(0);
                else if (e.key == 'd') snake0.set_direction(1);
                else if (e.key == 's') snake0.set_direction(2);
                else if (e.key == 'a') snake0.set_direction(3);
                else if (e.key == 'ArrowUp') snake1.set_direction(0);
                else if (e.key == 'ArrowRight') snake1.set_direction(1);
                else if (e.key == 'ArrowDown') snake1.set_direction(2);
                else if (e.key == 'ArrowLeft') snake1.set_direction(3);
            });

        } else {
            const [snake1] = this.snakes;
            this.ctx.canvas.addEventListener("keydown", e => {
                if (e.key == 'ArrowUp') snake1.set_direction(0);
                else if (e.key == 'ArrowRight') snake1.set_direction(1);
                else if (e.key == 'ArrowDown') snake1.set_direction(2);
                else if (e.key == 'ArrowLeft') snake1.set_direction(3);
            });
        }
    }
    check_ready() {
        for (const snake of this.snakes) {
            if (snake.status !== "idle") return false;
            if (snake.direction === -1) return false;
        }
        return true;
    }
    check_tail_increasing() {//检测当前回合，蛇的长度是否增加
        if (this.kind === 'double') {
            if (this.step % 3 === 1) return true;
            return false;
        } else if (this.kind === 'single') {
            if (this.check_eat(this.snakes[0].cells[0])) return true;
            return false;
        }
    }
    next_step()//让两条蛇进入下一回合
    {
        this.step++;
        for (const snake of this.snakes) {
            snake.next_step();
            if (this.kind === 'double') {
                snake.set_direction(-1);
            }
        }

    }
    check_valid(cell)//检测目标位置是否合法，没有撞到蛇的身体和墙
    {
        for (const wall of this.walls) {
            if (wall.r === cell.r && wall.c === cell.c) return false;
        }
        for (const snake of this.snakes) {
            let k = snake.cells.length;
            if (!this.check_tail_increasing())//当蛇尾会前进的时候，蛇尾就不用判断
            {
                k--;
            }
            for (let i = 0; i < k; i++) {
                if (snake.cells[i].r === cell.r && snake.cells[i].c === cell.c) return false;
            }
        }
        return true;
    }
    check_eat(cell) {
        if (this.score.r === cell.r && this.score.c === cell.c) {
            this.score.destroy();
            for (let i = 0; i < 1000; i++)//暴力生成score
            {
                if (this.create_score()) break;
            }
            return true;

        }
        return false
    }
    update() {
        this.update_size();
        if (this.check_ready()) {
            this.next_step();
        }
        this.render();

    }
    render() {
        const color_even = "#AAD751", color_odd = "#A2D149";
        for (let r = 0; r < this.rows; r++) {
            for (let c = 0; c < this.cols; c++) {
                if ((r + c) % 2 === 0) {
                    this.ctx.fillStyle = color_even;
                } else {
                    this.ctx.fillStyle = color_odd;
                }
                this.ctx.fillRect(c * this.L, r * this.L, this.L, this.L);
            }
        }

    }
}